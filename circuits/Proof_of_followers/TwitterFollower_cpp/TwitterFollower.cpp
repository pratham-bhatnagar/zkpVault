...
void Twitter_follower_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Twitter_follower_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterEqThan_2_run,
Twitter_follower_3_run };
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 46],&lvar[3]); // line circom 32
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 46]); // line circom 38
...
// load src
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 45]); // line circom 98
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
ctx->componentMemory[coffset].templateName = "Twitter_follower";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[2]); // line circom 15
...
Twitter_follower_3_create(1,0,ctx,"main",0);
Twitter_follower_3_run(0,ctx);
}
